...
#THIS CODE performs cleaning and feature addition in train_data and outputs train_data from Sept,2015 to April,2016.
...
train_data=train_data[(train_data.fecha_dato=='2016-04-28') | (train_data.fecha_dato=='2016-03-28') | (train_data.fecha_dato=='2016-02-28') | (train_data.fecha_dato=='2016-01-28') | (train_data.fecha_dato=='2015-12-28') | (train_data.fecha_dato=='2015-11-28') | (train_data.fecha_dato=='2015-10-28') | (train_data.fecha_dato=='2015-09-28')]
d1 = train_data[train_data.fecha_dato=='2016-04-28']
d2 = train_data[train_data.fecha_dato=='2016-03-28']
d3 = train_data[train_data.fecha_dato=='2016-02-28']
d4 = train_data[train_data.fecha_dato=='2016-01-28']
d5 = train_data[train_data.fecha_dato=='2015-12-28']
d6 = train_data[train_data.fecha_dato=='2015-11-28']
d7 = train_data[train_data.fecha_dato=='2015-10-28']
d8 = train_data[train_data.fecha_dato=='2015-09-28']
...
rename_dict={col:col+'_lag1' for col in prod_cols}
test_data.rename(columns = rename_dict, inplace = True)

#Adding toggle features for test_data
for c in prod_cols:
    sum00=np.zeros(len(test_data))
    sum01=np.zeros(len(test_data))
    sum10=np.zeros(len(test_data))
    sum11=np.zeros(len(test_data))
    for lag_no in range(1,7):
        x=test_data[c+'_lag'+str(lag_no)]
        y=test_data[c+'_lag'+str(lag_no+1)]
        x=x.fillna(0)
        y=y.fillna(0)
        x=x.astype(int)
        y=y.astype(int)
        ...
        sum01+=(x^y)&(y)		# will yield 1 only if x=0 and y=1, else 0.
        sum10+=(x^y)&(x)		# will yield 1 only if x=1 and y=0, else 0.
    test_data[c+'_sum00']=pd.Series(sum00)
    test_data[c+'_sum01']=pd.Series(sum01)
    test_data[c+'_sum10']=pd.Series(sum10)
    test_data[c+'_sum11']=pd.Series(sum11)
...
for c in prod_cols:
    for lag_no in range(4,8):
        test_data.drop([c+'_lag'+str(lag_no)],inplace=True,axis='columns')

# <-------------------------------Feature Addition done--------------------------->

test_data.drop('fecha_dato',axis='columns',inplace=True)
train_data.drop('fecha_dato',axis='columns',inplace=True)

#Output the train_data  and test_data to "train_data_second_half.csv" and "test_data.csv" respectively.
# This output .csv files in combination with 'train_data_first_half.csv' file will be used in model.py file 
train_data.to_csv('train_data_second_half.csv',index=False)
